/*  Frozander
    Main'in aynısı ama mainde kullanılmayacak testler içerir
    */
...
int main(int argc,char * argv[]){
  ...
  while(cevap!=0){
    ...
    if (cevap==1){
     ...
     else if (cevap==8) //SORTING TEST
     {
        
        House** ref = pull_from_table_by_id_as_headref(44, houses);
        print_list(*ref);
        merge_sort(ref, ID);
        print_list(*ref);
